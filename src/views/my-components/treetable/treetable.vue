        ...
        <tr v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
          <td v-for="(column,snum) in columns" :key="column.key" :style="tdStyle(column)">
            ...
            <div v-if="column.type === 'action'">
              <i-button :type="action.type" size="small" style="margin-right: 5px" @click="RowClick(item,$event,index,action.text)" v-for='action in (column.actions)' :key="action.text">{{action.text}}</i-button>
            </div>
            <label @click="toggle(index,item)" v-if="!column.type">
              <span v-if='snum==iconRow()'>
                <i v-html='item.spaceHtml'></i>
                <i v-if="item.children&&item.children.length>0" class="ivu-icon tree-icon" :class="{'ivu-icon-arrow-right-b':!item.expanded,'ivu-icon-arrow-down-b':item.expanded }"></i>
...
#hl-tree-table .table-bordered {
  border: 1px solid #EBEBEB;
}

#hl-tree-table>tbody>tr>td,
#hl-tree-table>tbody>tr>th,
#hl-tree-table>thead>tr>td,
#hl-tree-table>thead>tr>th {
...
#hl-tree-table.table-bordered>tbody>tr>td,
#hl-tree-table.table-bordered>tbody>tr>th,
#hl-tree-table.table-bordered>tfoot>tr>td,
#hl-tree-table.table-bordered>tfoot>tr>th,
#hl-tree-table.table-bordered>thead>tr>td,
#hl-tree-table.table-bordered>thead>tr>th {
...
#hl-tree-table .tree-icon {
  width: 14px;
  font-size: 14px;
}

#hl-tree-table .ms-tree-space {
  position: relative;
  top: 1px;
...
#hl-tree-table .ms-tree-space::before {
  content: ""
}
