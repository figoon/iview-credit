...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getSelectedNodes () {
        /* public API */
        let selectNode = this.flatState.filter(obj => obj.node.selected);
            
        if (selectNode.length>0) {
          let selectName = selectNode[0].node.title;

          if(isNaN(parseInt(selectNode[0].parent))) {
            return [selectName];
          } else {
            let parentKey = selectNode[0].parent,
                parentName = this.flatState[parentKey].node.title;
            return [parentName, selectName];
          }
        } else {
          return new Array(0);
        }
